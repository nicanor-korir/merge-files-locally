...
/** Wrap raw bytes as a File, the shape mergeDocuments() consumes. */
export function asEntry(bytes, name, type = 'application/pdf') {
  return { id: name, name, type, size: bytes.length, file: new File([bytes], name, { type }) };
}
