...
const compressImage = () => Promise.resolve({ bytes: PIXEL_JPEG, format: 'jpeg', passthrough: false });
...
describe('output settings', () => {
  it('fits pages to US Letter when asked', async () => {
    const source = asSource(await makePdf({ width: 300, height: 400 }), 'a.pdf');
    const out = await mergeDocuments(pagesOf(source, 1), {
      sources: [source],
      compressImage,
  ...
  it('leaves pages at their own size under "keep original"', async () => {
    const small = asSource(await makePdf({ width: 300, height: 400 }), 'small.pdf');
    const wide = asSource(await makePdf({ width: 842, height: 595 }), 'wide.pdf');
    const out = await mergeDocuments([...pagesOf(small, 1), ...pagesOf(wide, 1)], {
      sources: [small, wide],
      compressImage,
      pageSize: 'original',
    });
    ...
    expect(doc.getPage(1).getWidth()).toBeCloseTo(842, 1);
  });

  it('falls back to A4 for an unknown page size rather than failing', async () => {
    const source = asSource(await makePdf({ width: 300, height: 400 }), 'a.pdf');
    const out = await mergeDocuments(pagesOf(source, 1), {
      sources: [source],
      compressImage,
    ...
    expect((await reload(out.bytes)).getPage(0).getWidth()).toBeCloseTo(A4_WIDTH, 1);
  });

  it('embeds a lossless image through embedPng', async () => {
    const png = new Uint8Array(readFileSync(new URL('./__fixtures__/pixel.png', import.meta.url)));
    const source = asSource(png, 'shot.png', 'image/png');
    const out = await mergeDocuments(pagesOf(source, 1), {
      sources: [source],
      compressImage: () => Promise.resolve({ bytes: png, format: 'png', passthrough: true }),
    });
    expect((await reload(out.bytes)).getPageCount()).toBe(1);
  });

  it('re-encodes when passed-through bytes cannot be embedded', async () => {
    // A progressive JPEG decodes in a canvas but has no DCTDecode equivalent, so the direct
    // embed fails and the merge must retry rather than drop the page.
    const source = asSource(PIXEL_JPEG, 'photo.jpg', 'image/jpeg');
    const attempts = [];
    const flaky = (file, opts) => {
      attempts.push(Boolean(opts.forceCanvas));
      return Promise.resolve(
        opts.forceCanvas
          ? { bytes: PIXEL_JPEG, format: 'jpeg', passthrough: false }
          : { bytes: new Uint8Array([1, 2, 3]), format: 'jpeg', passthrough: true },
      );
    };
    const out = await mergeDocuments(pagesOf(source, 1), { sources: [source], compressImage: flaky });
    expect(attempts).toEqual([false, true]);
    expect((await reload(out.bytes)).getPageCount()).toBe(1);
  });

  it('skips just the one image whose re-encode also fails', async () => {
    const bad = asSource(PIXEL_JPEG, 'broken.jpg', 'image/jpeg');
    const good = asSource(PIXEL_JPEG, 'fine.jpg', 'image/jpeg');
    // Undecodable whichever way it is asked, so both the direct embed and the retry fail.
    const compress = (file) =>
      Promise.resolve(
        file === bad.file
          ? { bytes: new Uint8Array([1, 2, 3]), format: 'jpeg', passthrough: true }
    ...
    const out = await mergeDocuments([...pagesOf(bad, 1), ...pagesOf(good, 1)], {
      sources: [bad, good],
      compressImage: compress,
    });

    expect(out.skipped).toEqual([{ name: 'broken.jpg', reason: SKIP_UNREADABLE }]);
    // The good image still makes it through - one bad page never discards the batch.
    expect((await reload(out.bytes)).getPageCount()).toBe(1);
  });
});
