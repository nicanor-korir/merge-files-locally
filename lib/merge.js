...
import { applyPageTransform, fitPageToBox, fitToBox } from './pdf-geometry.js';
import { pageSizeOf, qualityOf } from './output-settings.js';
...
const embed = (merged, result) =>
  result.format === 'png' ? merged.embedPng(result.bytes) : merged.embedJpg(result.bytes);

async function appendImage(merged, source, page, compressImage, box, preset) {
  const request = { rotation: page.rotation, crop: page.crop, preset };
  let result = await compressImage(source.file, request);
  let image;

  try {
    image = await embed(merged, result);
  } catch (err) {
    // Passed-through bytes are the only ones we did not produce ourselves. PDF's DCTDecode
    // only understands baseline JPEG, so a progressive one decodes fine in a canvas but
    // cannot be embedded directly — re-encode rather than lose the page.
    if (!result.passthrough) throw err;
    result = await compressImage(source.file, { ...request, forceCanvas: true });
    image = await embed(merged, result);
  }

  const { width: imgW, height: imgH } = image.scale(1);
  const { drawW, drawH, pageW, pageH } = fitToBox(imgW, imgH, box);
 ...
 * @param {string} [options.pageSize] a PAGE_SIZES id; defaults to A4
 * @param {string} [options.quality] a QUALITY_PRESETS id; defaults to balanced
...
export async function mergeDocuments(pages, options = {}) {
  ...
  const box = pageSizeOf(options.pageSize);
  const preset = qualityOf(options.quality);
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    try {
      if (source.type === 'application/pdf') {
        ...
        fitPageToBox(copied, box);
        merged.addPage(copied);
      } else {
        await appendImage(merged, source, page, compressImage, box, preset);
      }
    } catch (err) {
...
export { describeSkipped } from './describe-skipped.js';
